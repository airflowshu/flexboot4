...
public class DefaultQueryWrapperBuilder extends AbstractQueryWrapperBuilder {
    ...
    private void applyItem(QueryWrapper qw, RelationQueryBuilder.RelationContext ctx, SearchDto.SearchItem it, String groupLogic) {
        ...
        if (it.getChildren() != null && !it.getChildren().isEmpty()) {
            log.debug("Processing item with children, groupLogic: {}, childrenLogic: {}", groupLogic, it.getLogic());
            if (isAnd) {
        ...
        if (it.getField() == null || it.getOp() == null) {
            log.debug("Skipping item: field={}, op={}", it.getField(), it.getOp());
            return;
        }
        ...
        log.debug("Processing field: {}, op: {}, val: {}, isAnd: {}", field, op, val, isAnd);
        if (field.contains(".")) {
            ...
            log.debug("Field contains dot: alias={}, rel={}, targetTable={}", alias, rel, targetTable);
            if (targetTable == null) {
            ...
            if (rel.isEmpty()) {
                ...
                log.debug("Matched as root entity field, applying condition: {}", col);
                if (isAnd) {
