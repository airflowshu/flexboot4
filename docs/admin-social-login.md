...
本文说明 Admin Starter 的第三方登录集成方式。第一版只实现 GitHub，后续接入其他第三方厂商时复用同一套后端 OAuth 回调、本地账号绑定、本地 JWT 登录模型。

## 1. 设计边界

第三方平台只作为身份认证来源，不直接决定系统权限。用户通过 GitHub 授权后，系统仍然只根据本地 `sys_user`、角色、菜单和权限码完成登录态与授权控制。

当前实现遵循以下规则：

- 不自动创建系统账号。
- 不根据 GitHub 邮箱自动绑定系统账号。
- GitHub verified email 只用于提示候选账号。
- 首次 GitHub 登录未绑定时，用户必须输入已有系统账号和密码完成绑定。
...
第三方登录不再设置 `thirdParty.enabled` 总开关，是否展示和是否允许发起授权只由 `providers[].enabled` 控制。`providers` 只表达运行开关，展示名称、图标等由前端按 `code` 映射。`clientSecret` 不能写入 `auth.login.options`，该配置会通过 `/api/admin/auth/options` 返回给前端。

当前初始化脚本默认保留 GitHub provider 配置，但不开启第三方登录：
...
```text
GET /api/admin/auth/oauth/github/authorize
```

后端生成 OAuth `state`，写入 Redis 后 302 跳转到 GitHub 授权页。

GitHub callback：

```text
GET /api/admin/auth/oauth/github/callback
```

后端校验 `state`，使用 GitHub `code` 换取 access token，然后读取 GitHub 用户信息和邮箱列表。GitHub access token 只在本次服务端调用中使用，不长期入库。
...
后端 callback 不把本地 JWT 或 GitHub access token 放进 URL，只把一次性结果票据 `ticket` 带到前端 `/auth/oauth/callback`。前端消费 `ticket` 后可能得到以下状态：

| 状态 | 含义 | 前端处理 |
| --- | --- | --- |
| `LOGIN_SUCCESS` | GitHub 身份已绑定启用的系统账号，且不需要 MFA | 写入本地登录态并进入系统 |
| `MFA_REQUIRED` | GitHub 身份已绑定系统账号，但该账号启用了 MFA | 弹出 MFA 验证，校验通过后进入系统 |
| `BIND_REQUIRED` | GitHub 身份未绑定系统账号 | 展示 GitHub 资料、候选账号和绑定表单 |
| `ERROR` | 授权失败、配置关闭、票据过期或其他拒绝场景 | 提示重新登录 |

绑定已有系统账号：

```text
POST /api/admin/auth/oauth/github/bind
```
...
| --- | --- |
| `user_id` | 本地系统用户 ID |
| `provider` | 第三方 provider，例如 `github` |
...
因此，一个 GitHub 身份只能绑定一个系统账号，一个系统账号当前只能绑定一个 GitHub 身份。若 GitHub 身份已绑定其他系统账号，或本地账号已经绑定另一个 GitHub 身份，绑定接口会拒绝。
...
解绑采用逻辑删除。解绑后，该 GitHub 身份再次登录会回到 `BIND_REQUIRED` 流程，需要重新输入系统账号密码完成绑定。
...
| Key 前缀 | 默认 TTL | 说明 |
| --- | --- | --- |
| `auth:oauth:state:` | 5 分钟 | GitHub authorize 到 callback 的 CSRF state |
| `auth:oauth:result:` | 3 分钟 | 后端 callback 跳回前端后的结果票据 |
| `auth:oauth:bind:` | 10 分钟 | 未绑定身份的绑定票据 |

安全注意事项：

- 不要把 GitHub `clientSecret` 写入系统配置表、前端环境变量或前端代码。
- 不要在 URL 中传递本地 JWT 或 GitHub access token。
- 生产环境必须使用 HTTPS。
- 反向代理部署时，需要正确传递 `Host`、`X-Forwarded-Proto`、`X-Forwarded-Host`，否则后端生成的 GitHub callback URL 可能与 OAuth App 配置不一致。
- GitHub OAuth App 的 callback URL、后端访问域名和前端实际发起登录请求的域名需要保持同一套环境，不要混用本地、测试和生产地址。
...
- `methods.thirdParty.providers` 是否包含 `code=github` 且 `enabled=true`。
...
### GitHub callback 失败或 state 过期

常见原因：

- GitHub OAuth App callback URL 与请求中的 `redirect_uri` 或后端 `FLEXBOOT4_AUTH_OAUTH_GITHUB_CALLBACK_URI` 不完全一致。
- Redis 不可用或临时 key 已过期。
- 浏览器重复打开同一个授权回调链接，第一次消费后后续会失败。
- 反向代理没有正确传递转发头，导致后端生成的 `redirect_uri` 与 GitHub OAuth App 中配置的不一致。

### 没有候选账号

系统只在 GitHub 返回 verified email，且本地启用用户存在同邮箱时显示候选账号。没有候选账号不影响绑定，用户仍可手动输入系统账号和密码。
