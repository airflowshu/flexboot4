...
public class LogAspect {
    ...
    private void setRequestValue(JoinPoint joinPoint, SysOperLog operLog, String[] excludeParamNames) {
        ...
        for (Object arg : args) {
            ...
            // 将 DTO 对象转换为 Map 以便后续脱敏处理
            if (arg instanceof Map<?, ?> mapArg) {
                arguments.add(mapArg);
            } else if (arg instanceof BindingResult result) {
                // BindingResult 包含错误信息，转换为 Map
                arguments.add(objectMapper.convertValue(result.getTarget(), new TypeReference<Map<String, Object>>() {}));
            } else {
                // 其他对象尝试转换为 Map
                try {
                    Map<String, Object> mapArg = objectMapper.convertValue(arg, new TypeReference<>() {});
                    arguments.add(mapArg);
                } catch (Exception e) {
                    // 转换失败，保留原值
                    arguments.add(arg);
